...
10. When a compatible tokenizer reports usage, call `token_shrinker_record_tokens` with raw/optimized counts, tokenizer ID, and `precision: "exact"`; otherwise preserve `estimated` measurements.
11. Call `token_shrinker_execute` only after the user approves the exact command and policy permits it.
12. Mark the task `complete` or `block` with `token_shrinker_task_update` before the final response.
13. Use `token_shrinker_format_final` only for the final human response. Never apply concise formatting to tool calls, JSON, code, commands, logs, citations, or intermediate evidence.
