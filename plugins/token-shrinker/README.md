...
The first MCP launch uses `npx` to obtain the pinned public package `@token-shrinker/cli@0.1.2` from npm. npm caches the package for later launches. Token-Shrinker does not redirect model-provider endpoints; repository context, memory, and execution remain local.
