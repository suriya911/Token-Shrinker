...
```bash
token-shrinker benchmark demo --output benchmarks/public-demo.json --json
```
...
```bash
# Verify the baseline-only installation, then produce raw JSON and Markdown.
token-shrinker doctor
token-shrinker benchmark demo --output benchmarks/public-demo.json --json
```

The checked-in [raw report](./benchmarks/public-demo.json) and [readable report](./benchmarks/public-demo.md) are generated by that command. The command exits unsuccessfully if any acceptance threshold fails.
...
See [SECURITY.md](./SECURITY.md) for vulnerability reporting and the [threat model](./docs/threat-model.md) for trust boundaries, mitigations, verification, and residual risk.
